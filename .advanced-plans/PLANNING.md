...
open_items:
  ...
  - "HAZARD, unresolved: the global ~/.claude layer is AHEAD of the advanced-planning source on 21 files and holds 17 files source has never seen - run-gate.md (+496 lines), phase-compact.md, sync-install.md, sync-plans.md, decompose-phase.md and all 7 gate agents. /sync-install copies source -> installed ONLY and states the reverse direction is unsupported, so running it today would overwrite roughly 2,000 lines of evolved command logic with stale source and offer no way back. The new install_audit classifies these DIVERGED rather than STALE precisely so /sync-install cannot pick them up, but the underlying reconciliation - adopting the installed versions back into source - has not been done and needs a decision. A full copy of the global layer is in this session scratchpad at global-claude-backup-2026-08-26/ (40 files)."
