...
open_items:
  ...
  - "RESOLVED 2026-08-26: the install-layer divergence hazard. The global ~/.claude layer was ahead of the advanced-planning source on 24 files and held 13 source had never seen; the source repo could not execute its own documented gate. Audited per-file (advanced-planning 9fb8e85, report at docs/install-layer-divergence-2026-08-26.md), then adopted installed -> source in 5722f58: 29 files, +3116/-331 - the plans/ -> .advanced-plans/ layout migration with its four missed lines fixed, ~1590 lines of real design (Codex preflight + parallel gate execution + conflict UX in run-gate, bounded remediation with SHA-256-frozen criteria in next-phase, branch scoping and mid-loop resume detection in next-loop, the incident-derived hard contracts, the re-gate isolation rule), and six framework orphans including sync-install.md itself. install_audit now reports 26 CURRENT, 7 STALE, 7 ORPHAN, ZERO DIVERGED - the 7 stale are source deliberately ahead by those fixes, the 7 orphans are other-project files (eddie x2, paper-shred-refiner, code-reviewer, plannotator x3). /sync-install is safe again in its intended direction and has NOT been run; running it would push the 7 fixes back into the global layer. Backup of the pre-adoption global layer (40 files) is in the session scratchpad at global-claude-backup-2026-08-26/."
  - "OPEN DECISION: the todo `complexity` field. The global todo.schema.md deleted it, but four files in the same global tree still specify complexity-driven model-tier selection - worker.md ('Haiku is appropriate when a todo's complexity field is low'), orchestrator.md and ralph-orchestrator.md (canonical field order still ... status -> complexity -> priority, contradicting the edited schema in its own tree), and agents/README.md (the cost rationale). An incomplete edit, not a design decision, so complexity was RETAINED in 5722f58 and only the skill-array half of that file adopted. Dropping it properly means 87 call-sites across the five live loop files here, 59 across 21 advanced-planning files, and retiring a dedicated 103-line docs/model-tier-strategy.md - about 146 sites in 26 files. Needs an explicit call before anyone touches it."
