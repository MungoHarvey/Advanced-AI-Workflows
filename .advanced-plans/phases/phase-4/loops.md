...
handoff_summary:
  ...
  needed: "The branch was pushed on 2026-08-26 under the loop-003-5 gate and is still unmerged, so main ships an incomplete install set until a PR lands; no PR has been opened and none is authorised. README:7 and SETUP.md:9 need one more edit when it lands. The AskUserQuestion contract gap in the glue skill is open. The test checks presence, not correctness — it cannot see that gap. .aaw/installed.json does not exist, so ACC-02 stale-data-directory detection is still wrong; that is loop 003."
