...
todos:
  ...
  - id: "loop-002-7"
    ...
    resolution: "CLOSED 2026-08-26 by the human gate. R1 accepted and fixed as a CLASS, not an instance - eleven sites in claude-md-routing.md now gate every companion-tool route on the manifest predicate with a named fallback, plus a default-deny catch-all that treats any future ungated route as gated anyway; 178 -> 213 lines, still pure LF, packaging 4/4 and idempotency 56/56 re-run clean. R2 accepted and narrowed to a spec location, a terminal state and a question format, with two stated absolutes. R3 DECLINED with reason - the line already leads with the generic harness wording and already carries a prose fallback, and the reviewer called it cosmetic. R4 folded into the re-prove rather than deferred. RE-PROVEN by three installer-built fixtures on one byte-identical envelope, no controller intervention between prompt and answer: all-installed-2 fires the routes and puts specs in .advanced-plans/specs (positive control); only-superpowers returns NONE for the gstack rules, writes to docs/superpowers/specs, and ignores a .advanced-plans decoy; broken-manifest returns NONE three times and reports components_installed: NONE despite EVERY sentinel being on disk and global gstack visible - the discriminator no earlier ACC round could build. Inputs byte-identical in all three. STILL OPEN and NOT closed by this: codex finding 3 stands - all seven ACC rounds measure the opencode harness only, so adherence on claude and codex is unmeasured. Publishing the mirror remains the user's to run."
    complexity: medium
    priority: high
