...
todos:
  ...
  - id: "loop-007-5"
    ...
    deferred_to: "phase-7"
    deferral_reason: "Half landed and half did not, and the half that did not is deliberately left rather than quietly closed. The second check is DISCHARGED and proven live: run-gate.md now instructs 'deferred', pinned to the schema enum by a test, and codex emitted a validating verdict under it in gate attempt 2. The first check is NOT discharged: next-loop.md Steps 6 and 7 still contradict core/agents/worker.md on who writes loop-complete.json. It is not closed here because reconciling them decides the worker/controller state contract for every adapter, which is a Phase 7 design question and not an edit - and because the user's own carve-out decision on loop-complete.json is the thing it has to be consistent with. Recorded as deferred WITH the reason rather than marked completed, because this todo was already once marked completed on the strength of a commit message rather than its checks, which is the exact defect this phase exists to remove. It is not, and must not become, a criterion deferred twice: criterion 3 does not depend on it - the discharged half is what criterion 3 needed."
    status: deferred
    complexity: medium
    priority: high
  - id: "loop-007-6"
    ...
    complexity: high
    priority: high
    result: "Criterion 1 replaced, not narrowed. Measured with per-surface canary tokens rather than a fake HOME, which loop-005-4 established defeats three of four hosts here. Check 1 (F20): OVERTURNED IN MECHANISM. opencode does load project skills - a unique-name control quoted canaries from both .agents/skills and .opencode/skills - but a global ~/.claude/skills skill of the same name beats the project copy, measured across three arms (default, skills.paths configured, .opencode/skills). Host behaviour we cannot configure; the collision itself is ours to detect. Check 2 (F21): claude reproduces, cursor does not apply - its mechanism is different. claude cannot be discriminated from workspace-trust without writing hasTrustDialogAccepted into the operator profile, which is outside this session authority and is loop-007-7 subject. Recorded un-discriminated with the reason. Check 3 (F23): the omission REPRODUCES 4/4, so it is not an unstable list. Mechanism found: cursor-agent --help has zero occurrences of skill, and cursor discovered nothing from .agents/skills, .claude/skills or .cursor/skills - it answered from the AGENTS.md fence, quoting our own README claim back at us. Check 4: four-surface digest table computed on a canary-free install. The two project surfaces are byte-identical on all seven shared names and match loop-005-4 exactly; the global copies, dated Jun 16, differ on six of seven, two substantively (plan-skill-identification frontmatter plus a body that predates the host-neutrality fix; plan-subagent-identification still naming Claude Code). Check 5: criterion 1 rewritten in plan.md with a dated note giving the reason. The rewrite moves the subject from host to adapter and keeps the failure the old criterion named. Adapter fix landed as advanced-planning a74f1a3: the gate Step 1 preflight audited --layers source,project, the layer the hosts were measured NOT to prefer, and now audits --layers all. Two mutation-checked regression tests added, one of which asserts the documented argument against install_audit own allow-set - the obvious --layers source,project,global is rejected with exit 2 and would have shipped as a silently failing preflight. Suite 1092 passed, 1 skipped."
    evidence_note: ".advanced-plans/evidence/2026-09-03-loop-007-6-host-discovery-measured.md"
  - id: "loop-007-7"
    ...
    status: completed
    result: "Criterion 2 has a measured answer on all four hosts. codex and cursor are NEWLY COMPLETE - one phase, one loop and one external task each, artefacts read back from disk by the controller, envelope validator re-run here rather than taken from the host's report and mutation-checked on the real artefact (drop base_sha -> rc=1, restore -> rc=0). Both envelopes are semantically truthful: codex base_sha 7e90a820 and cursor base_sha f2858433 each match their fixture's real HEAD, and base_ref master matches the real branch - materially better than opencode's loop-004-4 envelope, which validated while declaring a non-repository and forty zeros. Neither host was completed by answering a dialog: codex's blocker was quota (98% of the weekly window then, measured 9.0% today) and cursor's was the PowerShell hook wrapper recorded in 2026-09-01-cursor-hook-wrapper.md, cleared by LAUNCHING cursor-agent from PowerShell rather than by any permission grant, with a control probe returning the fixture's real HEAD before the run was trusted. claude is INCOMPLETE and recorded as a result with its blocker named, per check 4, not as a deferral: the claude-code adapter ships no shared router skill and no envelope writer, and its own installed .claude/commands/next-loop.md:355-359 says so - 'nothing in this adapter writes one... Migrating the adapter to emit an envelope is a Phase 7 item.' The envelope claude produced validates and names the real repo and HEAD, but the host chose the path itself because the installation defines none, and disclosed that in BLOCKED.md. Its step 4 was separately blocked by tool permission the controller could not pre-authorise: a scoped --allowedTools invocation was refused by the auto-mode classifier, correctly, and not routed around. Check 1's own text names the wrong two hosts (it says claude and cursor); the gate verdicts and the measurements say codex and cursor. Recorded rather than silently followed. Two by-products: F21 is SETTLED - with the l0076 fixture trusted the untrusted-workspace warning disappears and the description stays token-free, so claude prefers the global ~/.claude/skills copy on a name collision by PRECEDENCE, not trust, the same answer as opencode; and a shipping defect was found - the adapter's Write(.advanced-plans/**) and MultiEdit(.advanced-plans/**) allow rules are inert ('only Edit(path) rules are' matched), and the same four entries ship in setup-with-claude's settings snippet. Modes recorded per host. No push, tag, PR or repository write of any kind; two throwaway-fixture trust entries were written to ~/.claude.json under explicit operator authorisation, with the prior file backed up."
  ...
  - id: "loop-007-9"
    ...
    result: "Verdict is BOTH, and the split is the finding. Four of the five are a product defect - find_manifest checks for a manifest at ap_launcher.py:234 before the project boundary at :236 and the repo boundary at :238, so once the global record exists the upward walk reaches the profile and returns the GLOBAL record as though it were a project manifest. Boundary's own docstring (:190-200) had already ruled that out in writing: both boundary kinds mean stop looking upward for a project manifest, the global record is consulted next via resolve()'s explicit global_manifest() call at :298-299 and by no other route. The fifth, test_missing_manifest_is_reported_as_such, is a genuine isolation gap and now passes env=_clean_env. Worker 0f138de stops the walk at global_home(), returning None rather than raising Boundary - which mattered, since Boundary(home, project) would have failed the very test it was fixing, that test asserting no runtime.json at the boundary directory. Controller check 6 was NOT discharged by the worker: measured, with the fix removed and fixtures moved outside the profile via --basetemp, all 49 tests passed, so the four guards only guard where the profile holds a global record and pytest tmp_path sits beneath it - exactly the reliance on the developer's real profile check 6 forbade. Controller 9854ade adds test_the_walk_refuses_the_profile_record_on_any_machine, which builds a temporary profile holding a global record with an uninstalled project beneath it and points USERPROFILE/HOME at it; with the boundary removed and fixtures outside the profile it is the only failure of 50. Suite green for the first time since loop-007-6: 1098 passed, 1 skipped, and 1092 + 5 + 1 = 1098 so nothing was dropped to get there. CORRECTION 2026-09-03, made during the phase 6 gate: this result previously said the worker commit carried no opencode attribution trailer and that 05d1e55 did not either. Both statements were false - git log and git reflog show both trailers present at creation on both commits, unamended. The gate's code-review-agent caught it by opening the commits, which the controller had not done. The controller's own defect class, in the controller's own record."
    evidence_note: ".advanced-plans/evidence/2026-09-03-loop-007-9-profile-boundary.md"
    status: completed
    complexity: medium
    priority: high
...
todos:
  ...
  - id: "loop-008-9"
    ...
    result: "Run 2026-09-03 by opencode/Qwen worker gate-exit in herdr worktree loop-008-gate, commits 5a24129 (items 1-3) and d09d91f (the shipped prose, round 2), plus two controller commits 2d85287 and 1fb0f38; all four unpushed. All seven checks met, and the three criteria loop-008 left open are now ticked - 1408, 1409 (reworded first, see below) and 1411. The DECISION, made by the controller before dispatch and now recorded in main()'s docstring as a decision rather than a description: every way the gate can BLOCK shares exit code 1, because nothing in any shipped command branches on which gate failed, so separate codes would be a contract nothing consumes that a future edit could break in silence; the modes are separated by stderr prefix instead. Defect A closed: VACUOUS moved off path_scope: onto its own vacuous: prefix, so that prefix no longer matches both a real scope violation and the refusal to rule on zero paths. Defect B closed: the five git-subprocess failures moved from 1 to 2 under a git: prefix, because an environment failure is not a verdict about the operator's change - returning 1 there was this programme's own defect class inside the gate itself, an exit code asserting something never checked. The post-fix return map was measured by walking main(), not read off the diff: return 1 survives at exactly three sites (582 and 754 path_scope:, 732 vacuous:), return 2 at the five that moved (653 BAD REPO ROOT, 673/693/713 BAD BASELINE, 725 failed git diff) and everywhere else. Line 754 is the combined arm where schema errors, policy-gate reasons and scope violations all reach the same return under their own prefixes, which is why the record names four blocking modes and not three. Red-green was done controller-side BY REVERSION of the whole production file to 8cf17b5: three of the four new tests go red and the fourth, the positive control asserting a real scope violation still exits 1 under path_scope:, stays GREEN - that asymmetry is the whole point, since a gate that had started returning 2 for everything would have failed it. One existing test was edited, and only its exit-code assertion; its subject (BAD BASELINE not VACUOUS) is untouched, so it is a consequence rather than a weakening. An independent controller re-run with the TREE AS A PARAMETER scored 5/5 pre and 5/5 post over five scenarios including two controls; its first attempt measured a moving target (the live file the worker was still editing) and its second refused outright on an incomplete git archive rather than emit four meaningless rows, so the instrument caught itself twice before saying anything about the subject. Criterion 1408 was proven by EXECUTION at both sites, not by the rewording: the six-scenario harness re-ran 6/6 against a fresh installed fixture carrying the round-2 SKILL.md with runtime.json repointed at the branch tip, showing vacuous: through the shipped operator path end-to-end, and the resume site refuses to finalize and names the reason. Site selection stayed structural rather than positional and was unchanged by the doc edit, which is itself the check that the edit did not disturb extraction. Criterion 1409 was REWORDED before it was ticked, for the same reason 1387 was: as written it demanded the three modes be distinguishable BY EXIT CODE, which this loop deliberately decided against, and a criterion the code knowingly contradicts must be restated and re-checked rather than ticked as-is. A NEW FINDING drove round 2 and is the third instance in this programme of the same defect: the fix moved the reason out from under the documentation. Shipped SKILL.md said in two places that exit 2 should not happen with the shipped command, which item 3 had just made false in the commonest operator error there is, since step 7 makes the operator supply a baseline by hand. Both lines fixed; the collected-evidence arm correctly keeps malformed invocation because it makes no git calls. Three controller corrections to the round-2 output, none a defect in the fix: the worker EDITED line 166 when the envelope asked it only to check that line against the code - the edit is factually right (policy_gates does append to reasons and return 1, so the prose had omitted a blocking mode) but it was the wrong instruction followed; it reindented three bullets 3 to 4 spaces against the file's own style, reverted in 2d85287; and the round-1 decision record cited item 2, worker-envelope language a module reader cannot resolve, in the one paragraph whose whole job is to be legible years later, rewritten in 1fb0f38 with no behaviour change. Dispatch produced the false-success signature a third time: agent prompt returned agent_prompted with status done and state_change_seq unmoved at 1402, and the prompt had in fact landed, which a read moments later showed as working on seq 1421 with the tail of the prompt on screen. The worker's round-1 report also carried one false claim - hashes match before/after restore, written while listing two different hashes - where the underlying restore was correct and only the report was wrong, which is precisely the part a controller is tempted to read instead of the diff. Controller-run suite at the tip 1fb0f38: 1106 passed, 1 skipped in 361.89s, exit 0 captured before any pipe, against 1102 passed 1 skipped at loop-008-8 - the difference is the four new tests. Worker pane w2:p2H retired. Evidence: .advanced-plans/evidence/2026-09-03-loop-008-9-exit-codes-and-the-doc-that-outlived-them.md."
    status: completed
    gate: agent
  ...
  - [x] the shared router runs the gate as well as the schema validation, at BOTH sites, before the advancement is logged - or refuses to advance and says why, which is what the resume site does since F5, because the step-6 baseline SHA does not survive a crash and a guessed one would make the gate meaningless - proven by executing the shipped block in three scenarios
  - [x] the three failure modes are separable once the gate has RULED - they deliberately share exit code 1, because nothing in any shipped command branches on which gate failed, so separate codes would be a contract nothing consumes - and are distinguished instead by their stderr prefixes path_scope:, vacuous: and schema:, with the violating paths printed. An environment failure is separated by exit code rather than prefix: it returns 2 under git:, because the gate reached no verdict and exit 1 would assert something never checked
  - [x] a module named in a shipped command but lacking a __main__ fails a test derived from those commands
  - [x] validate_advancement and can_advance_loop are each exposed or deleted, and the decisions are recorded in the module
