...
>
> **Packaging blocker fixed on `feat/aaw-packaging-repair`.** The repository documented and installed `.claude/skills/gstack-to-plans/SKILL.md` without tracking it, so a fresh clone did not contain the documented install source. The skill is now tracked, and `tests/packaging/test-fresh-clone.sh` fails the build if any documented install source goes missing from a fresh clone again. On `main` the blocker still stands until this branch lands.
>
